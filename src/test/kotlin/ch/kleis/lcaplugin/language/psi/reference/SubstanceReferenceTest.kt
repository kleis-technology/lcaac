...
class SubstanceReferenceTest : ParsingTestCase("", "lca", LcaParserDefinition()) {
    ...
    fun test_getVariants() {
        ...
        } returns listOf("co2_air", "water_co2", "propanol_air", "water_propanol")

        // when
        val actual = ref.variants

        // then
        TestCase.assertEquals(2, actual.size)
        TestCase.assertEquals("co2_air", (actual[0] as LookupElementBuilder).lookupString)
        TestCase.assertEquals("water_co2", (actual[1] as LookupElementBuilder).lookupString)
